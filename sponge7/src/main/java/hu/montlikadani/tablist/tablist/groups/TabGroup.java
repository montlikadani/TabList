...
public class TabGroup implements Cloneable {
	...
	public void setTeam(final TabListUser user, int priority) {
		user.getPlayer().ifPresent(player -> {
			...
			final Text resultName = tl.getVariables().replaceVariables(player, tl.getVariables().setSymbols(pref + player.getName() + suf));

			tl.getTabUsers().forEach(users -> users.getPlayer().ifPresent(
					pl -> pl.getTabList().getEntry(user.getUniqueId()).ifPresent(te -> te.setDisplayName(resultName))));
		});
	}

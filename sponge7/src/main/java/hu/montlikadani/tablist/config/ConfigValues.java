...
public final class ConfigValues {
	...
	public static void loadValues(ConfigManager cm) {
		...
		try {
			timeFormat = DateTimeFormatter.ofPattern(cm.getString(node.getNode("time", "time-format", "time"), "mm:HH"));
		} catch (IllegalArgumentException e) {
		}

		cm.setComment(node.getNode("time", "date-format"), "Used for %date% placeholder");
		try {
			dateFormat = DateTimeFormatter.ofPattern(cm.getString(node.getNode("time", "date-format", "format"), "dd/MM/yyyy"));
		} catch (IllegalArgumentException e) {
		}
		...
		cm.setComment(setting = node.getNode("type"),
		...
		cm.setComment(setting = node.getNode("refresh-interval"), "The refresh interval when the objects are refreshing.");
